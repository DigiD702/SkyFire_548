            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_BANGING_THE_GONG:
                        ...
                        me->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY, ITEM_VIRTUAL_ITEM);
                        break;
                    ...
                    case EVENT_START_DOOR_OPENING_7:
                        if (Creature* hexLordTrigger = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_HEXLORD_TRIGGER)))
                            sCreatureTextMgr->SendChat(hexLordTrigger, SAY_HEXLORD_INTRO, NULL, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_MAP);
                        break;
                    default:
                        break;
                }
