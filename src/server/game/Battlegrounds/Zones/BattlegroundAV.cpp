...
BattlegroundAV::BattlegroundAV() : m_Mine_Timer(0), m_MaxLevel(0)
{
    ...
    BgCreatures.resize(static_cast<uint32>(AV_CPLACE_MAX) + static_cast<uint32>(AV_STATICCPLACE_MAX));
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type)
{
    ...
    ASSERT(type <= static_cast<uint16>(AV_CPLACE_MAX) + static_cast<uint16>(AV_STATICCPLACE_MAX));
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        ...
        SpawnBGObject(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH + static_cast<uint8>(GetTeamIndexByTeamId(owner)) + (2 * tmp), RESPAWN_ONE_DAY);
        SpawnBGObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH + static_cast<uint8>(GetTeamIndexByTeamId(owner)) + (2 * tmp), RESPAWN_ONE_DAY);
    }
    else
    {
        if (owner == ALLIANCE)
        ...
        SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + static_cast<uint8>(GetTeamIndexByTeamId(owner)) + 3 * node, RESPAWN_IMMEDIATELY);
        PopulateNode(node);
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    {
        SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node, RESPAWN_ONE_DAY);
        SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + static_cast<uint8>(GetTeamIndexByTeamId(team)) + 3 * node, RESPAWN_IMMEDIATELY);
    }
    // despawn old go
    SpawnBGObject(object, RESPAWN_ONE_DAY);
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    if (m_Nodes[node].TotalOwner != AV_NEUTRAL_TEAM)
    {
        ...
        else
        {
            ...
            SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node, RESPAWN_IMMEDIATELY); //neutral aura spawn
            SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + static_cast<uint8>(GetTeamIndexByTeamId(owner)) + 3 * node, RESPAWN_ONE_DAY); //teeamaura despawn
...
void BattlegroundAV::ResetBGSubclass()
{
    ...
    for (uint16 i = 0; i < static_cast<uint16>(AV_CPLACE_MAX) + static_cast<uint16>(AV_STATICCPLACE_MAX); i++)
        if (BgCreatures[i])
            DelCreature(i);
}
