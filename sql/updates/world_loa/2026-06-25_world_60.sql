-- LOA port: creature_model_info for map 1004 zone port models.
...
WHERE w.`modelid` IS NULL
  AND l.`modelid` IN (
      ...
      FROM (
          ...
          WHERE l2.`entry` IN (
              SELECT DISTINCT c.`id` FROM `creature` c WHERE c.`map` IN (1004)
          )
          UNION
          SELECT l2.`modelid2`
          FROM `loa`.`creature_template` l2
          ...
          WHERE c.`map` IN (1004) AND ct.`modelid1` > 0
          UNION
          SELECT ct.`modelid2`
          FROM `creature` c
          JOIN `creature_template` ct ON ct.`entry` = c.`id`
          WHERE c.`map` IN (1004) AND ct.`modelid2` > 0
          UNION
          SELECT ct.`modelid3`
          FROM `creature` c
          JOIN `creature_template` ct ON ct.`entry` = c.`id`
          WHERE c.`map` IN (1004) AND ct.`modelid3` > 0
          UNION
          SELECT ct.`modelid4`
          FROM `creature` c
          JOIN `creature_template` ct ON ct.`entry` = c.`id`
          WHERE c.`map` IN (1004) AND ct.`modelid4` > 0
      ) m
      WHERE m.`modelid` > 0
  );
